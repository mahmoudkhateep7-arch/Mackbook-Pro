...
Command: npx gltfjsx@6.5.3 macbook-14-transformed.glb 
...
export default function Model_Turning({ videoReff, setModel, model_ref, ...props }) {
  const internalRef = useRef(null)
  const { nodes, materials } = useGLTF('/models/macbook-14-transformed.glb')
  useFrame(() => {
    const trigger = ScrollTrigger.getById('main_scroll')
    if (trigger) {
      const scrollProgress = trigger.progress;
      const targetRotaion = scrollProgress * Math.PI * 4
      if (internalRef.current) {
        internalRef.current.rotation.y = targetRotaion
      }
    }
    setModel(true)
    ...
    >
      ...
      <mesh geometry={nodes.Object_123.geometry} material={materials.sfCQkHOWyrsLmor} rotation={[Math.PI / 2, 0, 0]} >
        <meshBasicMaterial toneMapped={false}>
          {videoReff?.current &&
            <videoTexture attach={'map'} args={[videoReff.current]}></videoTexture>
          }
        </meshBasicMaterial>
      </mesh>
